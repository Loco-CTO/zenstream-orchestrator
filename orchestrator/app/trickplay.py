...
class TrickplayStore:
    ...
    def queue_pending(self, library_id: str | None = None) -> int:
        ...
        rows = self.db.execute(
            ...
            "WHERE f.role=? AND s.video_codec IS NOT NULL AND s.video_codec<>''" + scope,
            params,
        )
        queued = 0
        for media_file_id, entity_id, file_hash, size, modified_ns in rows:
            ...
            if not existing:
                self.db.execute(
                    ...
                    "VALUES(?,?,?,?,?,?,?,?,?)",
                    (media_file_id, entity_id, fingerprint, width, height, interval, "queued", timestamp, timestamp),
                )
                queued += 1
                continue
            ...
            if old_fingerprint != fingerprint:
                self.db.execute(
                    ...
                    (entity_id, fingerprint, width, height, interval, timestamp, media_file_id),
                )
                queued += 1
        return queued
    ...
    def mark_ready(self, asset: dict, sheets: list[dict]) -> str | None:
        output_key = self.output_key(asset["fingerprint"], asset["width"], asset["height"], asset["intervalSeconds"])
        with self.db.transaction() as cursor:
            cursor.execute(
            ...
            if not current or current[0] != asset["fingerprint"]:
                return None
            cursor.execute("DELETE FROM trickplay_sheets WHERE media_file_id=?", (asset["mediaFileId"],))
            cursor.executemany(
                "INSERT INTO trickplay_sheets(media_file_id,output_key,sheet_index,first_frame,frame_count,relative_path) "
                "VALUES(?,?,?,?,?,?)",
                [
                    (asset["mediaFileId"], output_key, value["index"], value["firstFrame"], value["frameCount"], value["relativePath"])
                    for value in sheets
                ],
            )
            cursor.execute(
...
class TrickplayExtractor:
    ...
    def extract(self, asset: dict) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=root, prefix=".tmp-") as temporary:
            ...
            expected_frames = max(1, int(math.ceil(asset["durationSeconds"] / asset["intervalSeconds"])))
            sheets = [
                {
                    "index": index,
                    "firstFrame": index * FRAMES_PER_SHEET,
                    "frameCount": min(FRAMES_PER_SHEET, max(0, expected_frames - index * FRAMES_PER_SHEET)),
    ...
    def _remove_old_outputs(self, media_root: Path, current_key: str) -> None:
        ...
        for child in resolved.iterdir() if resolved.is_dir() else []:
            if child.name != current_key and child.is_dir() and not child.name.startswith("."):
    ...
    def remove_orphan_cache(self) -> None:
        ...
        if not root.is_dir():
            return
        candidates = [child for child in root.iterdir() if child.is_dir() and not child.name.startswith(".")]
        if not candidates:
            return
    ...
    def run(self, run_id: str, job_store, should_terminate=None) -> None:
        ...
        if should_terminate():
            ...
            job_store.update_run(run_id, state="failed", progress_current=completed, progress_total=max(completed + len(failures), discovered), finished_at=now(), message=summary, error=summary)
        else:
            job_store.update_run(
                run_id, state="completed", progress_current=completed, progress_total=max(completed, discovered), finished_at=now(),
                message=f"Extracted {completed} trickplay assets" if completed else "Trickplay sheets are current",
            )

    def manifest(self, user_id: str, entity_id: str, source_id: str | None = None) -> dict:
        rows = self.db.execute(
            ...
            + (" AND s.id=?" if source_id else "") + " ORDER BY f.size DESC LIMIT 1",
        ...
        base = {
            "state": state, "sourceId": selected_source_id, "mediaFileId": media_file_id,
            "frameWidth": width, "frameHeight": height, "intervalSeconds": interval,
            "columns": SHEET_COLUMNS, "rows": SHEET_ROWS, "durationSeconds": duration_seconds or 0,
        }
        if state != "ready" or not output_key:
            if state == "failed":
                raise HTTPException(422, {**base, "detail": error or "Trickplay extraction failed."})
            return base
        rows = self.db.execute(
