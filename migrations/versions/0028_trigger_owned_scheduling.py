...
def upgrade() -> None:
    ...
    if "options" not in trigger_columns:
        op.add_column("job_schedule_triggers", sa.Column("options", sa.Text(), nullable=False, server_default="{}"))
    run_columns = _columns("job_runs")
    if "source_trigger_id" not in run_columns:
        op.add_column("job_runs", sa.Column("source_trigger_id", sa.String(length=36), nullable=True))
    if "options" not in run_columns:
        op.add_column("job_runs", sa.Column("options", sa.Text(), nullable=False, server_default="{}"))

    definition_columns = _columns("job_definitions")
    if {"interval_minutes", "enabled"}.issubset(definition_columns):
        disabled = [row[0] for row in bind.execute(sa.text("SELECT id FROM job_definitions WHERE enabled=0"))]
        for definition_id in disabled:
            bind.execute(sa.text("DELETE FROM job_schedule_triggers WHERE definition_id=:id"), {"id": definition_id})
        # Preserve the existing metadata-refresh preference on every trigger.
        rows = bind.execute(sa.text("SELECT id,kind,config FROM job_definitions")).fetchall()
        for definition_id, kind, config_text in rows:
            ...
            if "preserveCachedAssets" not in config:
                continue
            options = json.dumps({"preserveCachedAssets": bool(config["preserveCachedAssets"])})
...
def downgrade() -> None:
    bind = op.get_bind()
    bind.execute(sa.text("ALTER TABLE job_definitions ADD COLUMN interval_minutes INTEGER NOT NULL DEFAULT 1440"))
    bind.execute(sa.text("ALTER TABLE job_definitions ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1"))
    bind.execute(sa.text("UPDATE job_definitions SET enabled=CASE WHEN EXISTS (SELECT 1 FROM job_schedule_triggers t WHERE t.definition_id=job_definitions.id) THEN 1 ELSE 0 END"))
    bind.execute(sa.text("UPDATE job_definitions SET interval_minutes=COALESCE((SELECT CAST(interval_seconds / 60 AS INTEGER) FROM job_schedule_triggers t WHERE t.definition_id=job_definitions.id AND t.trigger_type='interval' LIMIT 1),1440)"))
