...
class BackendSelectionTest(unittest.TestCase):
    ...
    def test_auto_is_native_even_for_shared_mounts(self):
        with patch.object(library_watcher, "_windows_remote_drive", return_value=True):
            self.assertEqual(
                choose_backend("/media", "auto"), ("native", "native_first_remote")
            )

    def test_environment_override(self):
        with patch.object(
            library_watcher, "_configured_backend", return_value="polling"
        ):
            self.assertEqual(
                choose_backend("/media", "auto"), ("polling", "environment_override")
            )
...
class ManagerStatusTest(unittest.TestCase):
    def test_native_event_marks_capability_verified(self):
        ...
        manager._statuses["library-1"] = WatchStatus(
            backend="native", state="active", capability="listening"
        )
        manager.emit("library-1", Path("/media"), ("/media/Show/Episode.mkv",))
        self.assertEqual(
            manager.status(
                {"id": "library-1", "type": "tv_series", "directory": "/media"}
            )["capability"],
            "verified",
        )
