...
def upgrade():
    ...
    if not columns:
        op.execute(sa.text("CREATE TABLE syncplay_members (group_id TEXT NOT NULL, user_id TEXT NOT NULL, participant_id TEXT NOT NULL DEFAULT '', username TEXT NOT NULL, viewing INTEGER NOT NULL DEFAULT 0, loading INTEGER NOT NULL DEFAULT 0, ready_generation INTEGER NOT NULL DEFAULT -1, presence_sequence INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (group_id, participant_id))"))
    elif "participant_id" not in columns or "user_id" in {row[1] for row in conn.exec_driver_sql("PRAGMA index_info(sqlite_autoindex_syncplay_members_1)")}:
        op.execute(sa.text("ALTER TABLE syncplay_members RENAME TO syncplay_members_legacy"))
        op.execute(sa.text("CREATE TABLE syncplay_members (group_id TEXT NOT NULL, user_id TEXT NOT NULL, participant_id TEXT NOT NULL DEFAULT '', username TEXT NOT NULL, viewing INTEGER NOT NULL DEFAULT 0, loading INTEGER NOT NULL DEFAULT 0, ready_generation INTEGER NOT NULL DEFAULT -1, presence_sequence INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (group_id, participant_id))"))
        participant = "CASE WHEN participant_id IS NULL OR participant_id = '' THEN '__legacy__:' || rowid ELSE participant_id END" if "participant_id" in columns else "'__legacy__:' || rowid"
        op.execute(sa.text(f"INSERT INTO syncplay_members SELECT group_id, user_id, {participant}, username, viewing, loading, ready_generation, presence_sequence FROM syncplay_members_legacy"))
