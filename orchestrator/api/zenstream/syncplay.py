...
from app.syncplay_socket import (
    broadcast_group,
    broadcast_group_ended,
    notify_participant_replaced,
)
...
def operation(data):
    return data.get("operationId") if isinstance(data.get("operationId"), str) else None


def expected(data):
    return (
        data.get("expectedRevision")
        if isinstance(data.get("expectedRevision"), int)
        else None
    )
...
def stale(error):
    return {"message": "Playback state is out of date.", "group": error.state}, 409
        ...
        try:
            ...
            def apply(cursor, state):
                ...
                cursor.execute(
                    ...
                    (group_id, user, participant, name()),
                )
            ...
            return {
                "message": "You must leave your current Syncplay group before joining another."
            }, 409
        except StaleSyncplayState as error:
            return stale(error)
        if replaced_participant:
            notify_participant_replaced(
                group_id, replaced_participant[0], state["revision"]
            )
        broadcast_group(state)
        return state, 200
        ...
        try:
            ...
            def apply(cursor, state):
                if state["hostUserId"] == user:
                    ...
                    cursor.execute(
                        "DELETE FROM syncplay_members WHERE group_id=? AND participant_id=?",
            ...
            return stale(error)
        if state["ended"]:
            broadcast_group_ended(group_id, state["revision"])
        else:
            broadcast_group(state)
        return "", 204
...
class Member(Resource):
    def delete(self, group_id, member_id):
        ...
        try:
            ...
            return {"message": "Only the host can remove members."}, 403
        except ValueError:
            return {"message": "The host cannot remove themselves."}, 400
        except StaleSyncplayState as error:
            return stale(error)
        broadcast_group(state)
        return state, 200
...
class Command(Resource):
    def post(self, group_id):
        ...
        if action not in {"media", "play", "pause", "seek"}:
            return {"message": "Invalid playback command."}, 400
        try:
            ...
            def apply(cursor, state):
                ...
                if requested and not waiting:
                    schedule(group, cursor, state, float(pos))
                elif requested:
                    group.transition(
                        cursor,
                        state,
...
class Presence(Resource):
    def post(self, group_id):
        ...
        if not isinstance(generation, int) or not isinstance(sequence, int):
            return {
                "message": "mediaGeneration and presenceSequence are required."
            }, 400
        ...
        def apply(cursor, state):
            if generation != state["mediaGeneration"]:
                return
            cursor.execute(
                "SELECT presence_sequence,watching_together FROM syncplay_members WHERE group_id=? AND participant_id=?",
                (group_id, participant),
            )
            row = cursor.fetchone()
            ...
            cursor.execute(
                "UPDATE syncplay_members SET viewing=?,loading=?,ready_generation=?,presence_sequence=? WHERE group_id=? AND participant_id=?",
                (
                    int(viewing),
                    int(loading),
...
class Participation(Resource):
    def post(self, group_id):
        ...
        if not isinstance(watching, bool):
            return {"message": "watchingTogether must be boolean."}, 400
        try:
            state = group.set_participation(
                user, participant, watching, operation(data)
            )
        except StaleSyncplayState as error:
            return stale(error)
        broadcast_group(state)
