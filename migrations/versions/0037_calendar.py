...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "provider IN ('sonarr','radarr')", name="ck_calendar_connections_provider"
        ),
        sa.CheckConstraint(
            "port BETWEEN 1 AND 65535", name="ck_calendar_connections_port"
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "provider IN ('sonarr','radarr')", name="ck_calendar_events_provider"
        ),
        sa.CheckConstraint(
            "kind IN ('episode','movie')", name="ck_calendar_events_kind"
        ),
        sa.CheckConstraint(
            "state IN ('future','existing')", name="ck_calendar_events_state"
        ),
    ...
    op.create_index(
        "ix_calendar_events_window", "calendar_events", ["library_id", "event_at"]
    )
    op.create_index(
    ...
    op.create_index(
        "ix_calendar_event_entities_entity", "calendar_event_entities", ["entity_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_calendar_event_entities_entity", table_name="calendar_event_entities"
    )
