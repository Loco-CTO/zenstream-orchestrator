...
export default function CalendarSettingsPage() {
	...
	async function remove(provider: Provider) {
		if (
			!session ||
			!window.confirm(
				`Remove ${provider === "sonarr" ? "Sonarr" : "Radarr"} calendar configuration?`,
			)
		)
			return;
		setSaving(provider);
		...
		if (response.ok) {
			...
			setMessage(
				`${provider === "sonarr" ? "Sonarr" : "Radarr"} configuration removed.`,
			);
		} else {
		...
		<SurfaceCard className="p-6">
			<div className="flex items-start justify-between gap-4">
				<div className="flex items-start gap-3">
					<div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#5ee3d8]/10 text-[#5ee3d8]">
						<Icon size={21} />
					</div>
					<div>
						<h2 className="text-xl font-bold">{title}</h2>
						<p className="mt-1 text-xs console-muted">
							{isSonarr ? "Episode air dates" : "Movie release dates"}
						</p>
					</div>
				</div>
				{service.validatedAt && (
			...
			<form
				onSubmit={(event) => onSave(event, provider)}
			...
			>
				<label className="block">
					...
					<input
						...
						onChange={(event) => onChange(provider, { address: event.target.value })}
				...
				<div className="grid gap-4 sm:grid-cols-[120px_1fr]">
					<label className="block">
						...
						<input
							...
							value={service.port}
							onChange={(event) =>
								onChange(provider, { port: Number(event.target.value) })
							}
							className="console-input mt-2 h-11 w-full rounded-xl px-4 text-sm outline-none"
						/>
					</label>
				...
				<label className="flex items-center gap-3 text-sm">
					<input
						type="checkbox"
						checked={service.useSsl}
						onChange={(event) => onChange(provider, { useSsl: event.target.checked })}
					/>
					Use SSL / HTTPS
				</label>
				<label className="block">
				...
				<label className="block">
					...
					<select
						value={service.libraryId}
						onChange={(event) =>
							onChange(provider, { libraryId: event.target.value })
						}
						className="console-input mt-2 h-11 w-full rounded-xl px-4 text-sm outline-none"
					>
					...
					{selectedLibrary && (
						<span className="mt-2 block text-xs console-muted">
							Calendar events are visible only to users granted this library.
						</span>
					)}
				</label>
				<div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/8 pt-5">
					{service.lastError ? (
						<span className="max-w-[70%] text-xs text-amber-200/80">
							Last sync: {service.lastError}
						</span>
					) : (
						<span className="text-xs console-muted">
							Daily sync at 03:00; future metadata refetch at 04:00.
						</span>
					)}
					<div className="flex items-center gap-2">
						{canRemove && (
							<button
								...
								className="material-icon-button text-red-300/80"
						...
						<button
							type="submit"
							disabled={saving}
							className="console-button-primary rounded-xl px-4 py-2 text-sm font-semibold"
						>
