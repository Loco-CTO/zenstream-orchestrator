...
_INDEXES = {
    "user_item_state": [
        ("idx_user_item_state_entity", ["entity_id"]),
        ("idx_user_item_state_user", ["user_id"]),
    ],
    ...
    "media_sources": [
        ("idx_media_sources_media_file", ["media_file_id"]),
        ("idx_media_sources_entity", ["entity_id"]),
    ],
    "collection_members": [("idx_collection_members_source", ["source_entity_id"])],
    "playback_sessions": [
        ("idx_playback_sessions_user_state", ["user_id", "state"]),
        ("idx_playback_sessions_expiry", ["expires_at"]),
    ],
    "trickplay_assets": [("idx_trickplay_assets_entity", ["entity_id"])],
    "trickplay_sheets": [("idx_trickplay_sheets_output", ["output_key"])],
    "intro_outro_assets": [
        ("idx_intro_outro_assets_entity", ["entity_id"]),
        ("idx_intro_outro_assets_season", ["season_id"]),
    ],
    "intro_outro_segments": [
        ("idx_intro_outro_segments_media_file", ["media_file_id"])
    ],
    "entity_provider_ids": [
        ("idx_entity_provider_ids_provider", ["provider", "provider_id"])
    ],
    "entity_person_credits": [
        ("idx_entity_person_credits_entity", ["entity_id"]),
        ("idx_entity_person_credits_person", ["person_id"]),
    ],
    ...
    "job_runs": [
        ("idx_job_runs_library_state", ["library_id", "state"]),
        ("idx_job_runs_definition", ["definition_id"]),
    ],
    "enrichment_queue": [
        ("idx_enrichment_queue_entity_state", ["entity_id", "state"]),
        ("idx_enrichment_queue_library_state", ["library_id", "state"]),
    ],
}
