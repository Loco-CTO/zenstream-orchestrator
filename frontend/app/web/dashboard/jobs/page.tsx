...
function taskGroup(job: Job) {
  ...
  if (job.kind.includes("trickplay") || job.kind.includes("intro_outro")) return "Media Analysis";
...
function progressFor(job: Job) {
  ...
  return Math.max(0, Math.min(100, (run.progressCurrent / run.progressTotal) * 100));
}
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    if (legacyId) {
      router.replace(`/web/dashboard/jobs/detail/?jobId=${encodeURIComponent(legacyId)}`);
      return;
    }
  ...
  async function toggleRun(task: Job, event: React.MouseEvent) {
    ...
    if (activeRun) {
      ...
      await adminFetch(`/api/admin/jobs/${task.id}/run`, session, { method: "POST" });
    }
    await load(session);
  }
  ...
  return (
    <div className="dashboard-page dashboard-design">
      ...
      {error && (
        <div role="alert" style={{ color: "var(--danger)", fontSize: 12, marginBottom: 16 }}>
          {error}
        </div>
      )}

      {loading ? (
        <div style={{ background: "#080808", borderRadius: 12, padding: "20px 22px", color: "#666", fontSize: 13 }}>
          Loading tasks…
        </div>
      ) : (
