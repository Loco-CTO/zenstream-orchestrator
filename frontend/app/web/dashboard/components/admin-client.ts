...
export async function adminFetch(path: string, session: Session, init: RequestInit = {}) {
  return fetch(path, { ...init, headers: { ...(init.headers || {}), Username: session.username, TOKEN: session.token } });
}
