...
class VersionTests(unittest.TestCase):
    def test_reads_main_counter(self):
        with patch("api.zenstream.version.Path.read_text", return_value=json.dumps({"main": 127})):
            self.assertEqual(_main_version(), 127)

    def test_invalid_counter_defaults_to_zero(self):
        with patch("api.zenstream.version.Path.read_text", return_value=json.dumps({"main": -1})):
